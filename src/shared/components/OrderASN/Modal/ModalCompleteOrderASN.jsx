...
const ModalCompleteOrderASN = (props) => {
  ...
  if (importProduct && importProduct.length !== 0) {
    mapImportProduct = importProduct.map((item, index) => ({
      ...
      ngay_Ke_Hoach: item.ngay_Ke_Hoach
        ? formarDateTimeddmmyyy(item.ngay_Ke_Hoach.slice(0, 10))
        : '',
    }));
  }
  ...
  return (
    <>
      ...
      <ModalDetailASN
        ...
        dateto={props.dateto}
        title="Chi tiết đơn hàng đã xử lý"
        id={autoId}
        arrTrangThai={[1, 3, 2]}
        inputType={1}
...
export default ModalCompleteOrderASN;
